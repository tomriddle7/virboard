...
function CalenderHeader({ submitOpen, updateAgency }: { submitOpen: (key: boolean) => void, updateAgency: (key: string) => void }) {
        ...
        <div className="relative text-2xl font-bold">
          ...
          {isOpen && (
            <ul className="absolute left-0 top-full mt-2 w-48 bg-white rounded-md shadow-lg py-1 z-10 text-base font-normal">
              ...
              {Array.from(headerAgencyMap.entries()).map(([key, value]) => (
                <li key={key}>
                  <button
